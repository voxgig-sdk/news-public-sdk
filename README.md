...
This API provides access to the latest news articles, allowing users to fetch and retrieve news data programmatically.

Learn more about News Public API at [news-public-api.onrender.com](https://news-public-api.onrender.com).

This is an unofficial SDK for the News Public public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Noticia — that you
...
const items = await client.Noticia().list()
```
...
```go
client := sdk.Test()
result, err := client.Noticia(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| **Noticia** | The Noticia entity (list). | `/api/noticias/` |
