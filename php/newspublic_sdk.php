...
class NewsPublicSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = NewsPublicHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = NewsPublicHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!NewsPublicFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, NewsPublicFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return NewsPublicUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = NewsPublicHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = NewsPublicHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = NewsPublicHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new NewsPublicError($op . "_allow",
                "NewsPublicSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = NewsPublicHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new NewsPublicError("graphql_error",
                "NewsPublicSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Noticia($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_noticia === null) {
                $this->_noticia = new NoticiaEntity($this, null);
            }
            return $this->_noticia;
        }
        return new NoticiaEntity($this, $data);
    }
