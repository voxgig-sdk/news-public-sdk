...
class NoticiaEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new NoticiaEntity($this->_client, $opts);
    }

    /**
     * @param Noticia|array $args Noticia data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NewsPublicHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Noticia|array The current Noticia data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Noticia fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NewsPublicHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Noticia fields).
     */
    ...
    /**
     * List Noticia items matching the given filter.
     *
     * @param NoticiaListMatch|array|null $reqmatch Match filter (any subset
     *   of Noticia fields) as an assoc-array; NoticiaListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Noticia[]|array A list of Noticia items as assoc-arrays at
     *   the SDK boundary; throws NewsPublicError on failure (item-5 convention).
     */
