...
{
    private static function utility(): NewsPublicUtility
    {
        return new NewsPublicUtility();
    }
    ...
    private static function ctx(array $over = []): NewsPublicContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new NewsPublicContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new NewsPublicOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof NewsPublicError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new NewsPublicOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = NewsPublicMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new NewsPublicOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = NewsPublicMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new NewsPublicOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = NewsPublicMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NewsPublicSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = NewsPublicMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new NewsPublicResponse([]),
            'result' => new NewsPublicResult([]),
        ]);
        [, $err] = NewsPublicMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new NewsPublicSpec([]),
            'response' => null,
            'result' => new NewsPublicResult([]),
        ]);
        [, $err] = NewsPublicMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new NewsPublicSpec(['step' => 's']),
            'response' => new NewsPublicResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new NewsPublicResult([]),
        ]);
        [, $err] = NewsPublicMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new NewsPublicSpec(['step' => 's']),
            'response' => new NewsPublicResponse(self::resp(200, ['v' => 1])),
            'result' => new NewsPublicResult([]),
        ]);
        [, $err] = NewsPublicMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new NewsPublicSpec(['step' => 's']),
            'response' => new NewsPublicResponse(self::resp(200, ['v' => 2])),
            'result' => new NewsPublicResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        NewsPublicMakeResponse::call($ctx);
    ...
    {
        $preset = new NewsPublicResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new NewsPublicSpec([]),
            'response' => new NewsPublicResponse([]),
            'result' => new NewsPublicResult([]),
        ...
        [$got, $err] = NewsPublicMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new NewsPublicResult([])]);
        [, $err] = NewsPublicMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new NewsPublicSpec([]), 'result' => null]);
        [, $err] = NewsPublicMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NewsPublicSpec(['step' => 's']),
            'result' => new NewsPublicResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new NewsPublicOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NewsPublicMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NewsPublicSpec(['step' => 's']),
            'result' => new NewsPublicResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new NewsPublicOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NewsPublicMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NewsPublicResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new NewsPublicSpec([]),
            'result' => new NewsPublicResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = NewsPublicMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NewsPublicMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new NewsPublicError('boom', 'boom');
        $utility->fetcher = function (NewsPublicContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NewsPublicSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = NewsPublicMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (NewsPublicContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = NewsPublicMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(NewsPublicResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        NewsPublicMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (NewsPublicContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NewsPublicSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = NewsPublicMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new NewsPublicResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new NewsPublicSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = NewsPublicMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = NewsPublicMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = NewsPublicMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new NewsPublicResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, NewsPublicDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new NewsPublicResult(['ok' => false])]);
        $this->expectException(NewsPublicError::class);
        NewsPublicDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new NewsPublicResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', NewsPublicMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        NewsPublicMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new NewsPublicBaseFeature();
        $b = new NewsPublicBaseFeature();
        NewsPublicFeatureAdd::call($ctx, $a);
        NewsPublicFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): NewsPublicBaseFeature
    {
        $f = new NewsPublicBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        NewsPublicFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): NewsPublicContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new NewsPublicSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NewsPublicPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        NewsPublicPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new NewsPublicResponse(['headers' => null]),
            'result' => new NewsPublicResult([]),
        ]);
        NewsPublicResultHeaders::call($ctx);
