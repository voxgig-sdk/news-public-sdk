...
class FtCtrl extends NewsPublicControl
{
...
class FtClient
{
    ...
    private NewsPublicUtility $utility;

    public function __construct(NewsPublicUtility $utility, array $options)
    {
    ...
    public function get_utility(): NewsPublicUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NewsPublicContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NewsPublicContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new NewsPublicUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (NewsPublicContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NewsPublicConfig::make_feature($name);
    ...
    private static function buildUrl(NewsPublicSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NewsPublicError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NewsPublicResponse($fetched);
        }
    ...
    private function fail(NewsPublicContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NewsPublicContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
