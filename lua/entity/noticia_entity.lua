...
local NoticiaEntity = {}
NoticiaEntity.__index = NoticiaEntity


function NoticiaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NoticiaEntity)
  self._name = "noticia"
  self._client = client
...
function NoticiaEntity:get_name()
  return self._name
end


function NoticiaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NoticiaEntity.new(self._client, opts)
end
...
function NoticiaEntity:mark_deleted()
  self._deleted = true
end


function NoticiaEntity:deleted()
  return true == self._deleted
end


function NoticiaEntity:data_set(args)
  if args ~= nil then
...
function NoticiaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NoticiaEntity:match_set(args)
  if args ~= nil then
...
function NoticiaEntity:match_get()
...
function NoticiaEntity:stream(action, args, callopts)
...
function NoticiaEntity:_run_op(ctx, post_done)
  local utility = self._utility
