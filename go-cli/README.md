...
boru-driven command-line client **and** interactive REPL for the NewsPublic
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/news-public-cli)
make build

# 2. See usage (words, entities, env vars)
./news-public-cli --help

# 3. Provide credentials once, via the environment
export NEWS_PUBLIC_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./news-public-cli list noticia

# 5. Override the API base URL for a single call
NEWS_PUBLIC_BASE=https://api.example.com ./news-public-cli list noticia

# 6. No arguments -> interactive REPL
./news-public-cli
news-public> list noticia
news-public> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/news-public-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/news-public-cli list noticia
   ```
...
Run with no arguments to open a REPL (prompt `news-public>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `noticia`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/news-public-cli`. |
...
(`./news-public-cli <expr>`) and interactively (the REPL), and expressions compose the
