...
require_relative 'NewsPublic_types'
...
class NewsPublicSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NewsPublicHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NewsPublicHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NewsPublicFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NewsPublicUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NewsPublicHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NewsPublicHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NewsPublicHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = NewsPublicHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NewsPublicError.new(
        "graphql_error", "NewsPublicSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
