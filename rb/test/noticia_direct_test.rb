...
class NoticiaDirectTest < Minitest::Test
  def test_direct_list_noticia
    setup = noticia_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-noticia", setup[:live] ? "live" : "unit")
    if _should_skip
...
def noticia_direct_setup(mockres)
  ...
  if live
    ...
    client = NewsPublicSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
