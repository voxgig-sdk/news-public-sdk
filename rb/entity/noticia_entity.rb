...
class NoticiaEntity
  ...
  def make
    opts = @_entopts.dup
    NoticiaEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NewsPublicHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Noticia, Hash] the current Noticia data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NewsPublicHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Noticia fields)
  def match_get
  ...
  #
  # @param reqmatch [NoticiaListMatch, Hash, nil] match filter (any subset of
  #   Noticia fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Noticia>, Array] the matching Noticia items; raises NewsPublicError on failure
