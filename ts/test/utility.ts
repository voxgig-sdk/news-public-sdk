...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NEWSPUBLIC_TEST_LIVE ||
    'TRUE' === process.env.NEWSPUBLIC_TEST_OVERRIDE
  ) {
  ...
  m.NEWSPUBLIC_TEST_EXPLAIN = process.env.NEWSPUBLIC_TEST_EXPLAIN || m.NEWSPUBLIC_TEST_EXPLAIN
