...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NEWS_PUBLIC_TEST_LIVE ||
    'TRUE' === process.env.NEWS_PUBLIC_TEST_OVERRIDE
  ) {
  ...
  m.NEWS_PUBLIC_TEST_EXPLAIN = process.env.NEWS_PUBLIC_TEST_EXPLAIN || m.NEWS_PUBLIC_TEST_EXPLAIN
