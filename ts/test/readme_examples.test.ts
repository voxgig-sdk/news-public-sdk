...
//     (\`new NewsPublicSDK(...)\` / \`NewsPublicSDK.test(...)\` become
//     \`NewsPublicSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"noticia":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NewsPublicSDK(...)\` and any
// \`NewsPublicSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NewsPublicSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NewsPublicSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
