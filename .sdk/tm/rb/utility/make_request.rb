...
module NewsPublicUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NewsPublicResponse.new({})
    ...
    if fetch_err
      ...
      response = NewsPublicResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NewsPublicResponse.new(fetched)
    else
