...
from newspublic_sdk.newspublic_types import (
    Noticia,
    NoticiaListMatch,
)
...
class NoticiaEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return NoticiaEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Noticia]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Noticia().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
