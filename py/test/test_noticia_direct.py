...
class TestNoticiaDirect:

    def test_should_direct_list_noticia(self):
        setup = _noticia_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-noticia", "live" if setup["live"] else "unit")
        if _skip:
...
def _noticia_direct_setup(mockres):
