...
class NewsPublicTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NewsPublicTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NewsPublicTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NewsPublicTestRunner.getenv("NEWS_PUBLIC_TEST_LIVE")
        override = NewsPublicTestRunner.getenv("NEWS_PUBLIC_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NewsPublicTestRunner.getenv(key)
        ...
        explain = NewsPublicTestRunner.getenv("NEWS_PUBLIC_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NEWS_PUBLIC_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NewsPublicTestRunner._test_control is not None:
            return NewsPublicTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NewsPublicTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NewsPublicTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NewsPublicTestRunner.load_test_control()
...
def load_env_local():
    NewsPublicTestRunner.load_env_local()


def env_override(m):
    return NewsPublicTestRunner.env_override(m)


def entity_data(v):
    return NewsPublicTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NewsPublicTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NewsPublicTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NewsPublicTestRunner.load_test_control()


def live_delay_ms():
    return NewsPublicTestRunner.live_delay_ms()
