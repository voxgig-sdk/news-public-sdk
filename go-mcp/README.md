...
[MCP](https://modelcontextprotocol.io) server exposing the NewsPublic SDK as
two agent tools — `news-public_list` and `news-public_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/news-public-mcp)
make build

# 2. Provide credentials via the environment
export NEWS_PUBLIC_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user news-public \
  -- /absolute/path/to/news-public-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./news-public-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// news-public_list: first page of records
{ "entity": "noticia" }
{ "entity": "noticia", "query": { } }

// news-public_load: one record by id
{ "entity": "noticia", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/news-public-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user news-public \
     -- "$PWD"/dist/*/news-public-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `news-public_list` and `news-public_load` tools now appear
   in new sessions. Ask the agent to *"list noticia using news-public"*
   and it calls `news-public_list` with `{"entity":"noticia"}`.
...
```sh
export NEWS_PUBLIC_APIKEY=sk_live_xxx            # API key
export NEWS_PUBLIC_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `news-public_list` tool
...
### Call the `news-public_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `news-public_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `news-public_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NEWS_PUBLIC_APIKEY` | API key sent with every request. |
| `NEWS_PUBLIC_BASE` | Optional override of the API base URL. |
...
```sh
./news-public-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"news-public_load","arguments":{"entity":"noticia","query":{"id":1}}}}'
```
