...
type Args struct {
	Entity string         `json:"entity" jsonschema:"noticia"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.NewsPublicSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "news-public_list",
		Description: "List records from NewsPublic. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "news-public_load",
		Description: "Load a single record from NewsPublic. " +
...
func runOp(client *sdk.NewsPublicSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.NewsPublicSDK, name string) (sdk.NewsPublicEntity, error) {
	switch strings.ToLower(name) {
	case "noticia":
		return client.Noticia(nil), nil
