...
func TestNoticiaEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Noticia(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Noticia(nil).Stream("list", nil, nil) {
...
func noticiaBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NEWS_PUBLIC_TEST_NOTICIA_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NEWS_PUBLIC_TEST_NOTICIA_ENTID"])
